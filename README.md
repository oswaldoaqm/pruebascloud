...
9. [Multi-nube: integración con OCI (Rappi)](#-multi-nube-integración-con-oci-rappi)
10. [Frontend](#-frontend)
11. [Seguridad](#-seguridad)
12. [Despliegue desde cero](#-despliegue-desde-cero)
13. [Suite de verificación (QA)](#-suite-de-verificación-qa)
14. [Observabilidad](#-observabilidad)
15. [Credenciales y URLs de demo](#-credenciales-y-urls-de-demo)
16. [Estructura del repositorio](#-estructura-del-repositorio)
17. [Decisiones de diseño y limitaciones conocidas](#-decisiones-de-diseño-y-limitaciones-conocidas)
...
| Si vino de Rappi, **actualizar estado en Rappi en cada paso** | Lambda `notify_rappi` (EventBridge, filtro `origin=RAPPI`) llama a la 2da API en OCI. |
...
**Más allá de lo pedido** (robustez y UX): consola de administración por sede (personal, productos, roles),
consola de **SUPERADMIN** de cadena con métricas y gestión de sedes, cancelación de pedidos (lado de
...
flowchart LR
  ...
  subgraph OCI["☁️ OCI (segunda nube)"]
    R1[API-1 Rappi Ingest<br/>contenedor FastAPI]
    R2[API-2 Rappi Status<br/>contenedor FastAPI + SQLite]
  end
...
| `order.step.changed` | `ms.workflow` | cada paso | `actualizar_status` (estado del pedido) · `notify_rappi` (si `origin=RAPPI`) |
| `order.completed` | `ms.workflow` | fin del flujo | `actualizar_status` (→ DELIVERED) · `notify_rappi` |
| `order.failed` | `ms.workflow` | timeout/cancelación | `actualizar_status` (→ FAILED) · `notify_rappi` |
...
- **API-2 `rappi-status`** (`:8001`): recibe de AWS la actualización de estado en **cada paso** y la
  persiste; expone un `GET /orders/{id}` para evidenciar el estado “en Rappi”.

La integración es por **contratos REST + API key**, totalmente desacoplada: AWS notifica vía EventBridge
→ `notify_rappi`. Detalle de despliegue en [`oci/README.md`](oci/README.md).
...
**`web-clientes`** — catálogo por categorías, carrito (drawer animado), login/registro, checkout y
**tracker del pedido** con stepper animado en tiempo real (polling), “Mis pedidos” y cancelación.
...
├── frontend/
│   ├── web-clientes/               # SPA cliente (catálogo, carrito, tracker)
│   └── web-trabajadores/           # consola operaciones + admin + superadmin
├── images/                         # catálogo: pizzas/ complementos/ bebidas/ postres/
├── oci/
│   ├── rappi-ingest/               # API-1 (Dockerfile + FastAPI)
│   ├── rappi-status/               # API-2 (Dockerfile + FastAPI + SQLite)
│   └── README.md                   # despliegue OCI
├── scripts/                        # suite de QA (run-all, check-health, smoke, security, multinube)
