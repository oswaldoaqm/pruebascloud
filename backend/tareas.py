...
def dashboard(event, context):
    ...
    OPTIMIZACIÓN: usa QUERY sobre el índice GSI2 (PK = TENANT#<t>) en lugar de un
    SCAN con filtro. El scan lee TODA la tabla (O(n) sobre todos los tenants) y luego
    descarta; la query va directo a la partición del tenant (O(items del tenant)),
    reduciendo latencia y RCUs consumidas, y manteniendo el aislamiento multi-tenant.
    """
    ctx = _auth(event)
    ...
    while True:
        kwargs = {
            "IndexName": "GSI2",
            "KeyConditionExpression": Key("GSI2PK").eq(f"TENANT#{ctx['tenant_id']}"),
        }
        if start_key:
